...
import pg from 'pg';
const { Client } = pg;
const connectionString = process.env.CONNECTION_STRING || process.env.NEW_CONNECTION_STRING;
if (!connectionString) {
    console.error('❌ CONNECTION_STRING or NEW_CONNECTION_STRING is required in env');
    process.exit(1);
}
